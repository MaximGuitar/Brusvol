...
class Content
{
	public function __construct($option = '', $content_type = 'content')
	{
		...
		$this->some_parameter = $option; //передаём параметры для вывода группы полей (ID)
		while (empty($option) ? have_rows($content_type) : have_rows($content_type, $option)) {
			...
			get_template_part("components/pages/content-blocks/$layout"); //создаём папку по этому пути и все блоки которые названием совпадают с гибким содержимым работают
		}
		echo '</div>';
	}
